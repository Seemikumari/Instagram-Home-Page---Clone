...
async function fetchRandomImages() {
  try {
    const response = await fetch('https://api.unsplash.com/photos/random?count=5&client_id=VNgBtvkMe0LNiYXcgwHbLZNcA8KE3NNj8RP14meK4S4');
...
// Function to create post data array
async function createPostData() {
  const images = await fetchRandomImages();
  const postData = [];

  // Generate random post data using the fetched images
  for (let i = 0; i < images.length; i++) {
    const username = `User ${i + 1}`;
    const likes = Math.floor(Math.random() * 100); // Generate random likes count
    const comments = Math.floor(Math.random() * 10); // Generate random comments count

    postData.push([
      `https://via.placeholder.com/150`, // Placeholder for profile picture
      username,
      likes,
      images[i], // Random image URL
      likes, // Same as likes count for demonstration
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit.', // Sample description
      comments // Random comments count
    ]);
  }
...
async function renderPosts() {
  const post_data = await createPostData();
        ...
        <div class="desc">
          <div class="icons">
              <div class="button-group">
                <button class="like-btn"><img src="./images/love.png"></button>
                <button class="comment-btn"><img src="./images/bubble-chat.png"></button>
                <button class="share-btn"><img src="./images/send.png"></button>
              </div>
          </div>
          <div class="liked">
